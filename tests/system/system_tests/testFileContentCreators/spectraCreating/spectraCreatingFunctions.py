...
def create_template_library_dataframe(minMz, maxMz, precursorMzDiff, peakMzDiff, peakIntensityDiff):
    data = []
    ...
    df = pd.DataFrame(data,
                      columns=['precursorMz', 'peptide', 'precursorCharge', 'id', 'protein', 'peakMz', 'peakIntensity'])
    return df
...
def add_query_file_components_to_mz_window_spectra_summary(mzWindowSpectraBreakdown):
    for mzWindowGroup in mzWindowSpectraBreakdown:
        mzWindowGroup['queryScanMz'], mzWindowGroup['windowWidth'] = find_mz_window_of_query_scan_from_spectra(mzWindowGroup['libSpectra'])
        mzWindowGroup['querySpectra'] = [create_query_spectrum_from_library_spectrum_and_cosine_score(spectrum, mzWindowGroup['cosine']) for spectrum in mzWindowGroup['libSpectra']]
...
spectrastColumns = ["PrecursorMz", "FullUniModPeptideName", "PrecursorCharge", "transition_group_id", "ProteinName", "ProductMz", "LibraryIntensity"]
