...
import numpy as np
import pytest
from . import create_template_library_dataframe, separate_target_and_decoy_library_spectra, make_mz_window_spectra_summary_for_library_spectra, add_query_file_components_to_mz_window_spectra_summary, spectrastColumns, make_output_df_from_spectra_breakdown
...
def libraryTemplateDataFrame():
    return create_template_library_dataframe(minMz = 400, maxMz = 1600, precursorMzDiff = 1, peakMzDiff = 0.1, peakIntensityDiff = 1000.0)

@pytest.fixture(scope="module")
def libraryFileDirectory(libraryTemplateDataFrame):
    libraryDirectory = TemporaryDirectory(prefix='csodiaq_system_test_library_files_')
    libraryTemplateDataFrame.to_csv(os.path.join(libraryDirectory.name, 'template_test_library.csv'), index=False)
    spectrastDataFrame = libraryTemplateDataFrame.copy()
    spectrastDataFrame.columns = spectrastColumns
    spectrastDataFrame.to_csv(os.path.join(libraryDirectory.name, 'spectrast_test_library.csv'), index=False)
    return libraryDirectory

@pytest.fixture(scope="module")
def librarySpectraBreakdown(libraryTemplateDataFrame):
    targetDecoySpectra = separate_target_and_decoy_library_spectra(libraryTemplateDataFrame)
...
def standardMzWindowSpectraBreakdown(librarySpectraBreakdown):
    return add_query_file_components_to_mz_window_spectra_summary(librarySpectraBreakdown)
...
def test__identification__standard_run(libraryFileDirectory, standardExpectedOutputDataFrame):
    inputQueryFile = os.path.join(get_file_from_system_test_folder('inputs'), 'query.mzXML')
    libraryFile = os.path.join(libraryFileDirectory.name, 'spectrast_test_library.csv')
