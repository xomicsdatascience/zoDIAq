...
class ProteinCosineEvalScoresBreakdown(MaccScoresBreakdown):
    def _create_input_template_for_scoring_module(self):
        ...
        """
        sameProteinLayerDf = self._create_two_peptides_from_same_protein_with_different_ranks('000','001',rank1=0, rank2=1)
    ...
    def _create_two_peptides_from_same_protein_with_different_ranks(self, peptide1AndProteinId, peptide2Id, rank1, rank2):
        peptides = [f'peptide{peptide1AndProteinId}', f'peptide{peptide2Id}']
        proteins = [f'1/protein{peptide1AndProteinId}', f'1/protein{peptide1AndProteinId}']
        charges = [1,1]
        isDecoy = [0,0]
        ranks = [rank1, rank2]
    ...
    def _make_expected_spectral_output(self, scoredDf):
        ...
        expectedSpectralOutput = expectedSpectralOutput.iloc[:204,:].reset_index(drop=True)
    ...
    def _make_expected_peptide_output(self, scoredDf):
        ...
        expectedPeptideOutput = expectedPeptideOutput.iloc[:204,:].reset_index(drop=True)
