...
class MaccScoresBreakdown(BaselineScoresBreakdown):
    def _create_input_template_for_scoring_module(self):
        ...
        duplicatePeptideInputDf = self._create_input_for_generic_peptides(rank=2, startingRow=0)
        ...
        extraSpectralInputDf = self._create_input_for_generic_peptides(rank=6, startingRow=200)
    ...
    def _make_expected_protein_output(self, peptideDf):
        ...
        leadingProteins[:len(self.idPickerLeadingProteins)] = self.idPickerLeadingProteins
        ...
        proteinDf["uniquePeptide"] = [0] * len(self.idPickerLeadingProteins) + [1] * (len(proteinDf.index) - len(self.idPickerLeadingProteins))
        return proteinDf