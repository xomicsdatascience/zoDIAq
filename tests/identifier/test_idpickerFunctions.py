import pandas as pd
from csodiaq.identifier.idpickerFunctions import (
    ...
    reduce__identify_minimum_number_of_most_connected_proteins,
...
def test__idpicker_functions__separate__identify_and_label_independent_clusters():
    ...
    df = pd.DataFrame(data, columns=["peptide", "protein"])
    expectedClusters = np.array([0, 1, 1, 2, 2, 2, 1, 1, 2, 2, 2, 1])
...
def test__idpicker_functions__reduce__identify_minimum_number_of_most_connected_proteins():
    ...
    df = pd.DataFrame(data, columns=["peptide", "protein"])
    clusters = np.array([0, 1, 1, 2, 2, 2, 1, 1, 2, 2, 2, 1])
    df["cluster"] = clusters
