...
def test__csodiaq_parser__set_args_from_command_line_input__gui__explicitly_flagged_succeeds(parser):
    args = vars(parser.parse_args(["gui"]))
    assert args["command"] == "gui"

def test__csodiaq_parser__set_args_from_command_line_input__gui__succeeds_with_no_command_provided(parser):
...
def test__csodiaq_parser__output_directory_parsing_class__rejects_file_as_input(testOutputDir):
...
def test__csodiaq_parser__output_directory_parsing_class__rejects_input_that_has_no_existing_parent_directory(testOutputDir):
...
def test__csodiaq_parser__output_directory_parsing_class__if_directory_exists_new_directory_made_in_directory(testOutputDir, outputDirectoryPattern):
...
def test__csodiaq_parser__output_directory_parsing_class__if_directory_does_not_exist_new_directory_made_with_header_at_end_of_given_path(testOutputDir, outputDirectoryPattern):
    testDirectory = TemporaryDirectory(prefix="csodiaq_input_test_directory_")
...
def test__csodiaq_parser__input_query_file_parsing_class__rejects_non_existing_file_values(testInputFile):
...
def test__csodiaq_parser__input_query_file_parsing_class__rejects_files_of_wrong_type(testInputFile):
...
def test__csodiaq_parser__library_file_parsing_class__rejects_non_existing_file_values(libraryFile):
    testFile = 'this/path/does/not/exist'
    errorOutput = "The -l or --library argument must be an existing file (and not a directory)."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        libraryFile(testFile)

def test__csodiaq_parser__library_file_parsing_class__rejects_files_of_wrong_type(libraryFile):
    testFile = NamedTemporaryFile(prefix='csodiaq_input_test_file_', suffix=".txt")
...
def test__csodiaq_parser__restricted_int_parsing_class__fails_when_value_cannot_be_assigned_to_int(restrictedInt):
    inputValue = "string"
    errorOutput = "The test argument must be an integer."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        restrictedInt(inputValue)

def test__csodiaq_parser__restricted_int_parsing_class__fails_when_value_is_below_min_value(restrictedInt):
    inputValue = "-3"
    errorOutput = "The test argument must be an integer greater than or equal to -2."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        restrictedInt(inputValue)

def test__csodiaq_parser__restricted_int_parsing_class__fails_when_value_is_above_max_value(restrictedInt):
    inputValue = "4"
    errorOutput = "The test argument must be an integer less than or equal to 3."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        restrictedInt(inputValue)

def test__csodiaq_parser__restricted_int_parsing_class__succeeds_when_value_is_equal_to_min_value(restrictedInt):
    inputValue = "-2"
    output = restrictedInt(inputValue)
    assert isinstance(output, int)
    assert output == -2

def test__csodiaq_parser__restricted_int_parsing_class__succeeds_when_value_is_greater_than_min_value_less_than_max_value(restrictedInt):
    inputValue = "0"
    output = restrictedInt(inputValue)
    assert isinstance(output, int)
    assert output == 0

def test__csodiaq_parser__restricted_int_parsing_class__succeeds_when_value_is_equal_to_max_value(restrictedInt):
    inputValue = "3"
    output = restrictedInt(inputValue)
    assert isinstance(output, int)
    assert output == 3
...
def test__csodiaq_parser__restricted_float_parsing_class__fails_when_value_cannot_be_assigned_to_float(restrictedFloat):
    inputValue = "string"
    errorOutput = "The test argument must be a float."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        restrictedFloat(inputValue)

def test__csodiaq_parser__restricted_float_parsing_class__fails_when_value_is_below_min_value(restrictedFloat):
    inputValue = "-3"
    errorOutput = "The test argument must be a float greater than or equal to -2."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        restrictedFloat(inputValue)

def test__csodiaq_parser__restricted_float_parsing_class__fails_when_value_is_above_max_value(restrictedFloat):
    inputValue = "4"
    errorOutput = "The test argument must be a float less than or equal to 3."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        restrictedFloat(inputValue)

def test__csodiaq_parser__restricted_float_parsing_class__succeeds_when_value_is_equal_to_min_value(restrictedFloat):
    inputValue = "-2"
    output = restrictedFloat(inputValue)
    assert isinstance(output, float)
    assert output == -2

def test__csodiaq_parser__restricted_float_parsing_class__succeeds_when_value_is_greater_than_min_value_less_than_max_value(restrictedFloat):
    inputValue = "0"
    output = restrictedFloat(inputValue)
    assert isinstance(output, float)
    assert output == 0

def test__csodiaq_parser__restricted_float_parsing_class__succeeds_when_value_is_equal_to_max_value(restrictedFloat):
    inputValue = "3"
    output = restrictedFloat(inputValue)
    assert isinstance(output, float)
    assert output == 3
...
def test__csodiaq_parser__identification_output_directory_parsing_class__fails_when_not_a_directory(identificationOutputDirectory):
    testFile = NamedTemporaryFile(prefix="csodiaq_test_file_", suffix=".txt")
    errorOutput = "The -i or --input argument must be a directory."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        identificationOutputDirectory(testFile.name)

def test__csodiaq_parser__identification_output_directory_parsing_class__fails_when_directory_has_no_csodiaq_identification_outputs(identificationOutputDirectory):
...
def test__csodiaq_parser__scoring_output_directory_parsing_class__fails_when_not_a_directory(scoringOutputDirectory):
    testFile = NamedTemporaryFile(prefix="csodiaq_test_file_", suffix=".txt")
    errorOutput = "The -i or --input argument must be a directory."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        scoringOutputDirectory(testFile.name)

def test__csodiaq_parser__scoring_output_directory_parsing_class__succeeds_when_directory_has_a_single_peptide_fdr_file(scoringOutputDirectory):
    testDir = TemporaryDirectory(prefix="csodiaq_test_directory_")
    testFile1 = NamedTemporaryFile(prefix="csodiaq_test_file1_", suffix=".txt", dir=testDir.name, delete=False)
...
def test__csodiaq_parser__scoring_output_directory_parsing_class__succeeds_when_directory_has_multiple_peptide_fdr_files(scoringOutputDirectory):
    testDir = TemporaryDirectory(prefix="csodiaq_test_directory_")
    testFile1 = NamedTemporaryFile(prefix="csodiaq_test_file1_", suffix=".txt", dir=testDir.name, delete=False)
    testFile2 = NamedTemporaryFile(prefix="csodiaq_test_file2_", suffix="peptideFDR.csv", dir=testDir.name, delete=False)
...
def test__csodiaq_parser__scoring_output_directory_parsing_class__succeeds_when_directory_has_a_single_protein_fdr_file(scoringOutputDirectory):
    testDir = TemporaryDirectory(prefix="csodiaq_test_directory_")
    testFile1 = NamedTemporaryFile(prefix="csodiaq_test_file1_", suffix=".txt", dir=testDir.name, delete=False)
...
def test__csodiaq_parser__scoring_output_directory_parsing_class__succeeds_when_directory_has_multiple_protein_fdr_files(scoringOutputDirectory):
    testDir = TemporaryDirectory(prefix="csodiaq_test_directory_")
    testFile1 = NamedTemporaryFile(prefix="csodiaq_test_file1_", suffix=".txt", dir=testDir.name, delete=False)
    testFile2 = NamedTemporaryFile(prefix="csodiaq_test_file2_", suffix="proteinFDR.csv", dir=testDir.name, delete=False)
...
def test__csodiaq_parser__scoring_output_directory_parsing_class__fails_when_directory_has_no_csodiaq_identification_outputs(scoringOutputDirectory):
    testDir = TemporaryDirectory(prefix="csodiaq_test_directory_")
    testFile1 = NamedTemporaryFile(prefix="csodiaq_test_file1_", suffix=".txt", dir=testDir.name, delete=False)
...
def test__csodiaq_parser__set_args_from_command_line_input__initialize_identification(parser, idArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_no_output_dir_provided(parser, idArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_no_input_file_provided(parser, idArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__id_succeeds_with_multiple_input_files(parser, idArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_no_library_file_provided(parser, idArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__id_succeeds_with_custom_match_tolerance(parser, idArgs):
    idArgs += ['-t', '10']
    args = vars(parser.parse_args(idArgs))
    assert args['matchTolerance'] == 10

def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_match_tolerance_less_than_1(parser, idArgs):
    idArgs += ['-t', '0']
    errorOutput = "The matchTolerance argument must be a float greater than or equal to 1."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        args = vars(parser.parse_args(idArgs))

def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_match_tolerance_greater_than_100(parser, idArgs):
    idArgs += ['-t', '61']
    errorOutput = "The matchTolerance argument must be a float less than or equal to 60."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        args = vars(parser.parse_args(idArgs))

def test__csodiaq_parser__set_args_from_command_line_input__id_succeeds_with_no_correction_flag(parser, idArgs):
    idArgs += ['-nc']
    args = vars(parser.parse_args(idArgs))
    assert args['noCorrection']

def test__csodiaq_parser__set_args_from_command_line_input__id_fails_with_no_correction_arguments_added(parser, idArgs):
    idArgs += ['-nc', 'randomBadValue']
    errorOutput = "unrecognized arguments: randomBadValue"
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        args = vars(parser.parse_args(idArgs))

def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_correction_degree_less_than_p5(parser, idArgs):
    idArgs += ['-c', '0.4']
    errorOutput = "The correctionDegree argument must be a float greater than or equal to 0.5."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        args = vars(parser.parse_args(idArgs))

def test__csodiaq_parser__set_args_from_command_line_input__id_fails_when_correction_degree_greater_than_2(parser, idArgs):
    idArgs += ['-c', '2.1']
    errorOutput = "The correctionDegree argument must be a float less than or equal to 2."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        args = vars(parser.parse_args(idArgs))

def test__csodiaq_parser__set_args_from_command_line_input__id_succeeds_with_custom_correction_degree(parser, idArgs):
    idArgs += ['-c', '1']
    args = vars(parser.parse_args(idArgs))
    assert args['correctionDegree'] == 1

def test__csodiaq_parser__set_args_from_command_line_input__id_succeeds_with_histogram_flag(parser, idArgs):
    idArgs += ['-hist']
    args = vars(parser.parse_args(idArgs))
    assert args['histogram']

def test__csodiaq_parser__set_args_from_command_line_input__id_fails_with_histogram_argument_added(parser, idArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__score_succeeds_with_cosine_input(parser, scoreArgs):
    scoreArgs += ['-s', 'cosine']
    args = vars(parser.parse_args(scoreArgs))
    assert args["score"] == "cosine"

def test__csodiaq_parser__set_args_from_command_line_input__score_succeeds_with_macc_input(parser, scoreArgs):
    scoreArgs += ['-s', 'macc']
    args = vars(parser.parse_args(scoreArgs))
    assert args["score"] == "macc"

def test__csodiaq_parser__set_args_from_command_line_input__score_fails_with_other_input(parser, scoreArgs):
...
def test__csodiaq_parser__set_args_from_command_line_input__targeted_reanalysis_fails_when_float_provided(parser, reanalysisArgs):
    reanalysisArgs += ['-p', '0.2']
    errorOutput = "argument -p/--protein: The protein argument must be an integer."
    with pytest.raises(argparse.ArgumentTypeError, match=re.escape(errorOutput)):
        args = vars(parser.parse_args(reanalysisArgs))

def test__csodiaq_parser__set_args_from_command_line_input__targeted_reanalysis_succeeds_with_custom_protein_input(parser, reanalysisArgs):
    reanalysisArgs += ['-p', '1']
    args = vars(parser.parse_args(reanalysisArgs))
    assert args['protein'] == 1

def test__csodiaq_parser__set_args_from_command_line_input__targeted_reanalysis_succeeds_with_heavy_isotope_flag(parser, reanalysisArgs):
    reanalysisArgs += ['-heavy']
    args = vars(parser.parse_args(reanalysisArgs))
    assert args['heavyIsotope']

def test__csodiaq_parser__set_args_from_command_line_input__targeted_reanalysis_fails_with_heavy_isotope_argument_added(parser, reanalysisArgs):
...
def test__csodiaq_parser__check_for_conflicting_args__presence_of_protein_arg_fails_if_no_protein_fdr_files_present(parser):
    testDir = TemporaryDirectory(prefix="csodiaq_test_directory_")
    peptideFile = NamedTemporaryFile(prefix="csodiaq_peptide_file_", suffix="peptideFDR.csv", dir=testDir.name, delete=False)
    reanalysisArgs = [
        "targetedReanalysis",
        "-i",
        testDir.name,
