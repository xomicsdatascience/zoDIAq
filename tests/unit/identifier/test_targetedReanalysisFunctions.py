...
import numpy as np
import pytest
from csodiaq.identifier.targetedReanalysisFunctions import (
    ...
    create_targeted_reanalysis_dataframe_by_compensation_voltage,
...
def test__output_formatting_functions__filter_out_peptides_based_on_user_settings__no_heavy_no_proteins(inputFilteringDf):
    isIncludeHeavy = False
    maximumPeptidesPerProtein = 0
    expectedOutputDf = inputFilteringDf.copy()
    outputDf = filter_out_peptides_based_on_user_settings(inputFilteringDf, isIncludeHeavy=isIncludeHeavy, maximumPeptidesPerProtein=maximumPeptidesPerProtein)
    assert expectedOutputDf.equals(outputDf)

def test__output_formatting_functions__filter_out_peptides_based_on_user_settings__is_heavy_no_proteins(inputFilteringDf):
    isIncludeHeavy = True
    maximumPeptidesPerProtein = 0
    ...
    expectedOutputDf = pd.DataFrame(expectedOutputData, columns=inputFilteringDf.columns)
    outputDf = filter_out_peptides_based_on_user_settings(inputFilteringDf, isIncludeHeavy=isIncludeHeavy, maximumPeptidesPerProtein=maximumPeptidesPerProtein)
    assert expectedOutputDf.equals(outputDf)

def test__output_formatting_functions__filter_out_peptides_based_on_user_settings__no_heavy_has_proteins(inputFilteringDf):
    isIncludeHeavy = False
...
def test__output_formatting_functions__filter_out_peptides_based_on_user_settings__is_heavy_has_proteins(inputFilteringDf):
    isIncludeHeavy = True
    maximumPeptidesPerProtein = 2
    expectedOutputData = [
        ["peptide04K", "protein1", 400.0, 1],
...
def test__output_formatting_functions__create_targeted_reanalysis_dataframe_by_compensation_voltage__no_heavy(inputFormattedDf, targetedReanalysisNoHeavyDf):
    ...
    output = create_targeted_reanalysis_dataframe_by_compensation_voltage(inputDf, isIncludeHeavy)
...
def inputProteinFdrDf():
    ...
    return pd.DataFrame(inputData, columns=["peptide","leadingProtein","ionCount","uniquePeptide","MzLIB","zLIB","CompensationVoltage"])


def test__output_formatting_functions__create_mass_spec_input_dataframes_for_targeted_reanalysis_of_identified_peptides__no_heavy_no_protein_no_cv(inputProteinFdrDf):
    isIncludeHeavy = False
    maximumPeptidesPerProtein = 0
    ...
    targetedReanalysisDf = pd.DataFrame(targetedReanalysisData, columns=["Compound","Formula","Adduct","m.z","z","MSXID"])
    expectedOutputDict = {
        "noCV": targetedReanalysisDf,
        "fullDf": fullDf,
    }
    outputDict = create_mass_spec_input_dataframes_for_targeted_reanalysis_of_identified_peptides(inputProteinFdrDf,isIncludeHeavy=isIncludeHeavy, maximumPeptidesPerProtein=maximumPeptidesPerProtein)
    for type, df in expectedOutputDict.items():
        assert type in outputDict
        assert df.equals(outputDict[type])

def test__output_formatting_functions__create_mass_spec_input_dataframes_for_targeted_reanalysis_of_identified_peptides__with_heavy_with_protein_with_cv(inputProteinFdrDf):
    isIncludeHeavy = True
    maximumPeptidesPerProtein = 2
    ...
    secondCvFilteredDf["leadingProtein"] = list(inputDfCV40.loc[filteredSortedIdxs]["leadingProtein"])
    ...
    outputDict = create_mass_spec_input_dataframes_for_targeted_reanalysis_of_identified_peptides(inputDf, isIncludeHeavy=isIncludeHeavy, maximumPeptidesPerProtein=maximumPeptidesPerProtein)
    expectedFullDf = expectedOutputDict["fullDf"].sort_values(["peptide","leadingProtein","MzLIB","CompensationVoltage"]).reset_index(drop=True)
    fullDf = outputDict["fullDf"].sort_values(["peptide","leadingProtein","MzLIB","CompensationVoltage"]).reset_index(drop=True)
    assert expectedFullDf.equals(fullDf)
    for type, df in expectedOutputDict.items():
        if type=="fullDf": continue
        assert type in outputDict
        assert df.equals(outputDict[type])