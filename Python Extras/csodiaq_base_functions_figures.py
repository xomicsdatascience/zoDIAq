...
'''
def library_file_to_dict(inFile, lp):
    fileType = inFile.split('.')[-1]
    if fileType == 'mgf':
        lib = mgf_library_upload(inFile, lp)
    else:
        lib = traml_library_upload(inFile, lp)
    return lib
...
'''
def traml_library_upload(fileName, lp):
    ...
    for key in lib:
        ...
        if len(peaks) > lp: peaks = peaks[:lp]
