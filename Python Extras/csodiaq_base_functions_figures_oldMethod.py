...
'''
def mgf_library_upload(fileName, libPeaks):
    ...
    for spec in libMGF:
        ...
        peaks = list(tuple(zip(mz,intensity,keyList)))
        peaks.sort(key=lambda x:x[1],reverse=True)
        if len(peaks) > libPeaks: peaks = peaks[:libPeaks]
...
'''
def pooled_spectra_analysis(expSpectraFile, outFile, lib, ppmTol, ppmYOffset, queryPooling, corrected, histFile, minMatch):
    ...
    with mzxml.read(expSpectraFile, use_index=True) as spectra:
        ...
        for spec in spectra:
            ...
            queValDict[scan]['precursorMz'] = precMz
            queValDict[scan]['windowWideness'] = window
            spectralCount += 1
        ...
        idToKeyDict = {}
        idToDecoyDict = {}
        for key in allLibKeys: idToKeyDict[lib[key]['ID']] = key; idToDecoyDict[lib[key]['ID']] = lib[key]['Decoy']
        ...
        for precMz_win, scans in queScanDict.items():
            ...
            for i in range(len(scans)):
                ...
                if (i % queryPooling == 0 and i!=0) or i == len(scans)-1:
                    ...
                    remove, maccScores = reduce_final_df(matchLibTags, matchLibIntensities, matchQueTags, matchQueIntensities, ppmMatches, minMatch)
                    ...
                    finalQueIntensities = np.append(finalQueIntensities, matchQueIntensities)
    ...
    tempFile = re.sub('(.*)_corrected.csv', r'\1_full.csv', outFile)

    with open(tempFile, 'w', newline='') as csvFile:
        writer = csv.writer(csvFile)
        writer.writerow(columns)

        if len(finalLibTags) != 0:
            ...
            count = 1
            length = len(finalLibTags)
            for i in range(1,length):
                if finalLibTags[i] != curLibTag or finalQueTags[i] != curQueTag:
                    if count > minMatch-1:
                        curMaccScore = finalMaccScores[i-1]
                    ...
                    curLibTag = finalLibTags[i]
                    curQueTag = finalQueTags[i]
                    curIonCount = finalQueIntensities[i]
                    count = 1
                else:
                    curIonCount += finalQueIntensities[i]
                    count += 1
            curMaccScore = finalMaccScores[-1]
            if count > minMatch-1:
                libKey = idToKeyDict[finalLibTags[-1]]
                scan = str(finalQueTags[-1])
                temp = [
                    expSpectraFile, #fileName
                    scan, #scan
                    queValDict[scan]['precursorMz'], #MzEXP
    ...
    if len(finalLibTags) != 0:
    #get list of library tags that work
    #alter collect_ppm_values to only include values if they are from those tags

        df = pd.read_csv(tempFile)
        hits, decoys = fdr_calculation(df)
        bestHits = 0
        bestMatch = -1
        limit = max(df['shared'])
        for i in range(3,limit+1):
            temp = df.sort_values('cosine', ascending=False).drop_duplicates(subset='peptide', keep='first').reset_index(drop=True)
            temp = temp[temp['shared'] > i-1].reset_index(drop=True)
            hits, decoys = fdr_calculation(temp)
            if hits > bestHits: bestHits = hits; bestMatch = i
        df = df.sort_values('cosine', ascending=False).drop_duplicates(subset='peptide', keep='first').reset_index(drop=True)
        df = df[df['shared'] > bestMatch-1].reset_index(drop=True)
        hits, decoys = fdr_calculation(df)
        df = df.loc[:hits]
        validTags = set(df['name'])



        # calculating offset and tolerance
        ppms = collect_ppm_values(finalPpmMatches, finalLibTags, validTags) #requires loop
        ...
        # All PSMs with fewer than 3 matches are excluded
        remove, finalMaccScores = reduce_final_df(finalLibTags, finalLibIntensities, finalQueTags, finalQueIntensities, finalDecoys, minMatch)
    ...
    # DELETE
    tempFile = re.sub('(.*).csv', r'\1_full.csv', outFile)
...
'''
@njit
def reduce_final_df(matchLibTags, matchLibIntensities, matchQueTags, matchQueIntensities, ppmMatches, matchNum):
    ...
    for i in range(1,length):
        if matchLibTags[i] != curLibTag or matchQueTags[i] != curQueTag:
            ...
            if count > matchNum-1:
                ...
                returnMaccScores.extend([cosine]*count)
...
'''
@njit
def collect_ppm_values(ppmMatches, libTags, validTags):
    ppms = []
    length = len(ppmMatches)
    for i in range(1,length):
        if libTags[i] in validTags: ppms.append(ppmMatches[i])
    return ppms
